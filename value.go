...
func (c *chunk) String() string {
	...
	log.Debugf("as----> %v", as)
	s := strings.Join([]string{c.flag.String(), "[", strings.Join(as, ", "), "]"}, "")
	log.Debugf("sss --> %s", s)
	return s
}
...
func (b *buffer) push(flag chunkFlag, args ...interface{}) tboValue {
	log.Debugf("push flag = %v   buffer = %s", flag, b)
	...
	case chunkStruct:
		...
		case '!':
			b.ignore()
			if b.to(structBegin) != b.c.args[0] {
...
func (b *buffer) pop(link bool) *chunk {
	...
	if c != nil {
		...
		b.send = c.snapshot.(*bufferSnapshot).send
	}
...
func (bt baseType) Value(v tboValue) {
	...
	case "uint":
		if _, err := strconv.ParseUint(s, 10, 64); err != nil {
			// @todo  {attack_time; [2]} 解析这种类型错误
			// fmt.Println("bad uint s:", s, "len s ==", len(s))
			if s == "" {
				s = "0"
			} else {
