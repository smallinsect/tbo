...
func init() {
	...
	RegisterPrinter("csharp", func() Printer {
		p := new(csharpStructPrinter)
		p.i = 0
		p.s = "csharp"
		p.basePrinter.c = p
		return p
...
func (p *goStructPrinter) PrintStruct(s *structType) {
	p.structs = append(p.structs, s.name)
	//fmt.

	p.Write([]byte("\ntype "))
	p.printRecordName([]byte(s.name))
	p.Write([]byte(" struct{\n"))
	p.Write(sepB)
	bi := bytes.Repeat(indentation, 2)
	b := false
	...
	for _, f := range s.fileds {
		...
		p.Write(bn)
		rnum := (len(bn)/16)*16 + 16

		p.Write(bytes.Repeat([]byte(" "), rnum-len(bn)))
		p.printStructFiledType(f.t)
		p.Write([]byte("  `json:\"" + f.name + "\"`  "))
		desc = f.desc
	}
	if len(desc) > 0 {
		p.Write([]byte(" // "))
...
func (p *goStructPrinter) printStructFiledType(t tboType) {
	...
	case *mapType:
		...
		p.printStructFiledType(tt.value)
	case *structType:
		p.printRecordName([]byte(tt.name))
	case *structIndex:
		p.printRecordName([]byte(tt.t.name))
	case *setType:
		p.printStructFiledType(tt.t)
	default:
...
func (p *goStructPrinter) PrintChunk(c *chunk) {
	defer func() {
		if r := recover(); r != nil {
			if e, ok := r.(chunkError); ok {
				panic(e)
	...
	case chunkSet:
		...
		for _, cc := range c.cs[0].cs {
			cname := cc.cs[0]
			name := strings.Title(toString(cname.cs[0].args[1]))
			p.Write(toBytes(name))
			p.Write([]byte("  int64\n"))
		}
		p.Write([]byte(" }{\n"))
		for _, cc := range c.cs[0].cs {
	...
	case chunkEnumElement:
		...
		p.i--
	case chunkEnumName:
		name := strings.Title(toString(c.cs[0].args[1]))
		p.Write(toBytes(name))
		p.Write([]byte("  int64"))
...
func (p *goStructPrinter) PrintBase(t string, v interface{}) {
	switch t {
	case "string":
		p.Write([]byte(fmt.Sprintf("%q", v)))
	case "bytes":
...
type csharpStructPrinter struct {
	basePrinter
}

func (p *csharpStructPrinter) CheckChunk(c *chunk) bool {
	if c == nil || c.flag != chunkSets {
		return false
	}
	for _, cc := range c.cs {
...
func (p *csharpStructPrinter) CheckStruct(s *structType) bool {
	return true
}

func (p *csharpStructPrinter) PrintHead() {
	// p.Write([]byte("using Newtonsoft.Json;\n"))
	p.Write([]byte("using System.Collections.Generic;\n"))
	p.Write([]byte("\nnamespace ConfigTable"))
	p.Write([]byte("\n{\n"))
}

func (p *csharpStructPrinter) PrintEnd() {
	p.Write([]byte("\n}"))
}

func (p *csharpStructPrinter) printRecordName(n []byte) {
	if p.e != nil {
		log.Debugf("p.v :%v", p.e)
		n = bytes.Join([][]byte{[]byte(p.e.prefix(p.s)), n, []byte(p.e.suffix(p.s))}, sepB)
	}
...
func (p *csharpStructPrinter) PrintStruct(s *structType) {
	//fmt.

	p.Write(sepB)
	bi := bytes.Repeat(indentation, 2)
	p.Write(bi)
	p.Write([]byte("public class "))
	p.printRecordName([]byte(s.name))
	p.Write([]byte("\n"))
	p.Write(bi)
	p.Write([]byte("{\n"))

	pi := bytes.Repeat(indentation, 2)
	for _, f := range s.fileds {
		if f.ext != nil && f.ext.ignore(p.s) {
			continue
		}
		...
		p.Write(pi)
		bn := []byte(f.name)
		// p.Write([]byte(fmt.Sprintf("[JsonProperty(\"%s\")]\n", f.name)))
		p.Write(pi)
		p.Write([]byte("public "))
		p.printStructFiledType(f.t)
		p.Write([]byte(" "))
		p.Write(bn)
		p.Write([]byte(" { get; set; } "))
		if len(f.desc) > 0 {
			p.Write([]byte(" // "))
			p.Write([]byte(f.desc))
		}
...
func (p *csharpStructPrinter) printStructFiledType(t tboType) {
	switch tt := t.(type) {
	case baseType:
		switch tt.String() {
		case "int":
	...
	case *setType:
		p.printStructFiledType(tt.t)
	default:
		p.Write([]byte("dynamic"))
	}
}

func (p *csharpStructPrinter) PrintBase(t string, v interface{}) {
	switch t {
	case "string":
		p.Write([]byte(fmt.Sprintf("%q", v)))
	case "bytes":
...
func (p *csharpStructPrinter) PrintChunk(c *chunk) {
	defer func() {
		if r := recover(); r != nil {
			if e, ok := r.(chunkError); ok {
				panic(e)
	...
	case chunkSet:
		...
		p.Write(bi)
		p.Write([]byte("public class "))
		c.cs[0].setName = string(toBytes(c.args[0]))
		p.printRecordName(toBytes(string(toBytes(c.args[0])) + "Class"))
		p.Write([]byte("\n"))
		p.Write(bi)
		p.Write([]byte("{\n"))
		p.i++
	...
	case chunkArray:
		...
		p.i++
		pi := bytes.Repeat(indentation, p.i)
		bn := []byte(toString(c.cs[0].args[0]))
		p.Write(pi)
		p.Write([]byte("\n"))
		p.Write(pi)
		p.Write([]byte("public List<"))
		p.Write([]byte(strings.Title(string(bn))))
		p.Write([]byte("> "))
		p.Write([]byte(c.setName))
		p.Write([]byte(" { get; set; } "))
		p.Write([]byte("\n"))
		p.i--
		p.i--
	case chunkEnumElement:
		...
		p.i--
	case chunkEnumName:
		name := toString(c.cs[0].args[1])
		pi := bytes.Repeat(indentation, p.i)
		p.Write([]byte("\n"))
		p.Write(pi)

		p.Write(pi)
		p.Write([]byte("public int "))
		p.Write(toBytes(name))
		p.Write([]byte(" { get; set; }"))
